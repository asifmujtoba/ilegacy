...
function handleCopyToClipboard() {
    [...document.querySelectorAll('#copytoclipboard')].map(elem=>{
        elem.addEventListener('click', function(e){
            e.preventDefault();
            var value = e.currentTarget.dataset.phoneno;
...
function handleNote() {
    document.querySelector('.saveNoteButton').addEventListener('click', async function () {
        const height = window.pageYOffset;
        currentNote = document.querySelector('textarea.notes').value
        leadId = document.querySelector('textarea.notes').dataset.leadid
...
function handleNoteButton() {
    ...
    [...document.querySelectorAll('.noteButton')].map(function (elem) {
        elem.addEventListener('click', function (e) {
            ...
            leadId = e.currentTarget.dataset.leadid;
            document.querySelector('textarea.notes').value = currentNote
            document.querySelector('textarea.notes').dataset.leadid = leadId
        });
    }, false);
}
        ...
        elem.addEventListener('click', function (e) {
            ...
            data.map(field => {
                if (document.querySelector(`.${field[0]}`)) {
                    if(field[0] ==='note'){
                        $note_text = field[0] && field[1].indexOf('||') ? field[1].split('||')[0] : field[1];
                        document.querySelector(`.${field[0]}`).value = $note_text;
                    }else{
...
function handleEdit() {
    document.querySelector('.saveEditButton').addEventListener('click', async function () {
        ...
        data.quantity = document.querySelector('#editFields.quantity').value
        data.price = document.querySelector('#editFields.price').value
        data.note = document.querySelector('#editFields.note').value
